...
def read_itv_area(
    ...
    """Read all ITV object-wise annotations in a given Area directory.
    ...
    batch = Batch.from_data_list(starmap_with_kwargs(
        read_itv_room, args_iter, kwargs_iter, processes=processes))
...
class ITV(BaseDataset):
    """ITV dataset, for Area-wise prediction.

    Note: we are using the ITV version with non-aligned rooms, which
...
class MiniITV(ITV):
    """A mini version of ITV with only 1 area per stage for
    experimentation.
    """
    _NUM_MINI = 1
